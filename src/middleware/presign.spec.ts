...
describe('presign middleware', () => {
  ...
  it('valid presigned GET returns file content', async () => {
    ...
    const res = await app.request(`/ps-bucket/file.txt?${makePresignedQuery(3600)}`)
  ...
  it('valid presigned PUT writes a file', async () => {
    ...
    const res = await app.request(`/ps-bucket2/upload.txt?${makePresignedQuery(3600)}`, {
      method: 'PUT',
      body: 'uploaded',
    })
    expect(res.status).toBe(200)
  ...
  it('expired presigned request returns 403 RequestExpired', async () => {
    ...
    const res = await app.request(`/ps-bucket/file.txt?${makePresignedQuery(3600, -7200 * 1000)}`)
